...
for ( this.g in seq(.25,.75, length.out = 3) )  {
  ...
  test_that( paste0( "svychu g=", this.g , " works on unweighted designs"), {
    expect_false( is.na ( coef( svychu( ~api00, design=dstrat1 , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" , deff = TRUE ) ) ) )
    expect_false( is.na ( SE( svychu( ~api00, design=dstrat1 , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" , deff = TRUE ) ) ) )
  } )
