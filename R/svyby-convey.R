    ...
    if ( "svyrep.design" %in% class(design) ) {
      ...
      if (keep.var) {
        ...
        if ((covmat && inherits(design, "svyrep.design")) || return.replicates) {
          ...
          covmat.mat <- svrVar(replicates, attr( design , "full_design" )$scale, attr( design , "full_design" )$rscales,
                               mse = attr( design , "full_design" )$mse, coef = as.vector(sapply(results, coef)))
        } else {
      ...
      if (return.replicates) attr(rval, "replicates") <- replicates
      ...
      if (keep.var) {
        ...
        if (covmat || influence) {
          ...
          for (i in seq_along(infs)) {
            inflmats[[i]] <- matrix(0, ncol = NCOL(infs[[i]]),
                                    nrow = length(idxs[[i]]))
