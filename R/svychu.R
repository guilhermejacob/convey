...
#' svychu(~eqincome, des_eusilc ,  abs_thresh=10000)
#' # poverty threshold equal to arpt
#' svychu(~eqincome, des_eusilc , type_thresh= "relq", thresh = TRUE)
#' # poverty threshold equal to 0.6 times the mean
#' svychu(~eqincome, des_eusilc , type_thresh= "relm" , thresh = TRUE)
#' # using svrep.design:
#' # poverty threshold fixed
#' svychu(~eqincome, des_eusilc_rep  ,  abs_thresh=10000)
#' # poverty threshold equal to arpt
#' svychu(~eqincome, des_eusilc_rep  , type_thresh= "relq", thresh = TRUE)
#' # poverty threshold equal to 0.6 times the mean
#' svychu(~eqincome, des_eusilc_rep  , type_thresh= "relm" , thresh = TRUE)
#'
#' \dontrun{
#'
...
#'
#' # poverty threshold fixed
#' svychu(~eqincome, dbd_eusilc ,  abs_thresh=10000)
#' # poverty threshold equal to arpt
#' svychu(~eqincome, dbd_eusilc , type_thresh= "relq", thresh = TRUE)
#' # poverty threshold equal to 0.6 times the mean
#' svychu(~eqincome, dbd_eusilc , type_thresh= "relm" , thresh = TRUE)
#'
...
svychu.survey.design <-
  function(formula, design, g, type_thresh="abs",  abs_thresh=NULL, percent = .60, quantiles = .50, thresh = FALSE, na.rm = FALSE, deff= TRUE , ...){
    ...
    if ( is.character(deff) || deff) {
      ...
      if (deff == "replace") vsrs <- svyvar( chulin , full_design, na.rm = na.rm) * npop^2/nobs
      else vsrs <- svyvar( chulin , full_design , na.rm = na.rm ) * npop^2 * (npop - nobs)/(npop * nobs)
...
svychu.svyrep.design <-
  function(formula, design, g, type_thresh="abs", abs_thresh=NULL, percent = .60, quantiles = .50, thresh = FALSE, na.rm = FALSE , deff = FALSE , ...) {
    ...
    if ( is.character(deff) || deff ) {
      ...
      # compute poverty measure influence function
      ...
      vsrs <- unclass( svyvar( chulin , full_design, na.rm = na.rm, return.replicates = FALSE, estimate.only = TRUE)) * npop^2/nobs
