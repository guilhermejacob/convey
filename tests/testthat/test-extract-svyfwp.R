...
# return test context
context("foster-wolfson index output survey.design and svyrep.design")
...
# perform tests
test_that("svyfwp works on unweighted designs", {
  expect_false( is.na ( coef( svyfwp( ~api00, design=dstrat1 ) ) ) )
  expect_false( is.na ( SE( svyfwp( ~api00, design=dstrat1 ) ) ) )
} )
...
# filter domains with enough sample size
des_eusilc <- subset( des_eusilc , hsize < 7 )
des_eusilc_rep <- subset( des_eusilc_rep , hsize < 7 )

# calculate estimates
a1 <- svyfwp( ~eqincome , des_eusilc , deff = TRUE , influence = TRUE )
a2 <- svyby( ~eqincome , ~hsize, des_eusilc, svyfwp , deff = TRUE , influence = TRUE , covmat = TRUE )
b1 <- svyfwp( ~eqincome , des_eusilc_rep , deff = TRUE , influence = TRUE )
b2 <- svyby( ~eqincome , ~hsize, des_eusilc_rep, svyfwp , deff = TRUE , covmat = TRUE )
...
test_that("database svyfwp",{
  ...
  # calculate estimates
  c1 <- svyfwp( ~ eqincome , dbd_eusilc , deff = TRUE , influence = TRUE )
  c2 <- svyby( ~ eqincome , ~hsize , dbd_eusilc , FUN = svyfwp , deff = TRUE , influence = TRUE , covmat = TRUE )
...
# calculate estimates
sub_des <- svyfwp( ~eqincome , design = subset( des_eusilc , hsize == 1) , deff = TRUE , influence = TRUE )
sby_des <- svyby( ~eqincome, by = ~hsize, design = des_eusilc, FUN = svyfwp , deff = TRUE , covmat = TRUE )
sub_rep <- svyfwp( ~eqincome , design = subset( des_eusilc_rep , hsize == 1) , deff = TRUE , influence = TRUE )
sby_rep <- svyby( ~eqincome, by = ~hsize, design = des_eusilc_rep, FUN = svyfwp , deff = TRUE , covmat = TRUE )
...
test_that("dbi subsets equal non-dbi subsets",{
  ...
  # calculate estimates
  sub_dbd <- svyfwp( ~eqincome , design = subset( des_eusilc , hsize == 1) , deff = TRUE , influence = TRUE )
  sby_dbd <- svyby( ~eqincome, by = ~hsize, design = des_eusilc, FUN = svyfwp , deff = TRUE , influence = TRUE , covmat = TRUE )
  sub_dbr <- svyfwp( ~eqincome , design = subset( des_eusilc_rep , hsize == 1) , deff = TRUE , influence = TRUE )
  sby_dbr <- svyby( ~eqincome, by = ~hsize, design = des_eusilc_rep, FUN = svyfwp , deff = TRUE , covmat = TRUE )
