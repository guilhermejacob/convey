...
for ( this.g in seq(.25,.75, length.out = 3) )  {
  ...
  # return test context
  context( paste0( "chu-relq g=", this.g , " output survey.design and svyrep.design" ) )
  ...
  test_that( paste0( "svychu g=", this.g , " works on unweighted designs"), {
    expect_false( is.na ( coef( svychu( ~api00, design=dstrat1 , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" ) ) ) )
    expect_false( is.na ( SE( svychu( ~api00, design=dstrat1 , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" ) ) ) )
  } )
  ...
  # calculate estimates
  a1 <- svychu( ~eqincome , des_eusilc , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
  a2 <- svyby( ~eqincome , ~hsize, des_eusilc , svychu , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
  b1 <- svychu( ~eqincome , des_eusilc_rep , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
  b2 <- svyby( ~eqincome , ~hsize, des_eusilc_rep , svychu , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
  ...
  test_that( "output svychu" , {
    ...
    expect_lte( se_diff2 , max( coef(a2) ) * .10 )  # the difference between CVs should be less than 5% of the maximum coefficient, otherwise manually set it
  ...
  test_that("database svychu",{
    ...
    # calculate estimates
    c1 <- svychu( ~eqincome , dbd_eusilc , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
    c2 <- svyby( ~eqincome , ~hsize, dbd_eusilc , svychu , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
  ...
  # calculate estimates
  sub_des <- svychu( ~eqincome , design = subset( des_eusilc , hsize == 1) , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
  sby_des <- svyby( ~eqincome, by = ~hsize, design = des_eusilc, FUN = svychu , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
  sub_rep <- svychu( ~eqincome , design = subset( des_eusilc_rep , hsize == 1) , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
  sby_rep <- svyby( ~eqincome, by = ~hsize, design = des_eusilc_rep, FUN = svychu , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
  ...
  test_that("dbi subsets equal non-dbi subsets",{
    ...
    # calculate estimates
    sub_dbd <- svychu( ~eqincome , design = subset( dbd_eusilc , hsize == 1) , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
    sby_dbd <- svyby( ~eqincome, by = ~hsize, design = dbd_eusilc, FUN = svychu , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
    sub_dbr <- svychu( ~eqincome , design = subset( dbd_eusilc_rep , hsize == 1) , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
    sby_dbr <- svyby( ~eqincome, by = ~hsize, design = dbd_eusilc_rep, FUN = svychu , g = this.g , quantiles = 0.5, percent = 0.6 , type_thresh = "relq" )
