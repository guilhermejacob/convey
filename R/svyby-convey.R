...
# convey.design svyby method
#' @importFrom survey svyby
#' @method svyby convey.design
#' @export
svyby.convey.design <-
  function (formula, by, design, FUN, ..., deff = FALSE, keep.var = TRUE,
    ...
    # add variables to database-backed designs
    if ( ( "DBIsvydesign" %in% class(design) ) & !( "logical" %in% class(attr(design, "full_design"))) ){
      ...
      # add full design information
      class( full_design ) <- setdiff( class( full_design ) , "convey.design" )
      class( full_design ) <- setdiff( class( full_design ) , "DBIsvydesign" )
    ...
    if ( "svyrep.design" %in% class(design) ) {
      ...
      if (inherits(by, "formula")) byfactors <- model.frame(by, model.frame(design), na.action = na.pass) else byfactors <- as.data.frame(by)
      if (return.replicates) {
        if (!inherits(design, "svyrep.design")) stop("return.replicates=TRUE not implemented for this design type")
      }
      if (multicore && !requireNamespace("parallel", quietly = TRUE)) multicore <- FALSE
      if (!inherits(formula, "formula")) {
        if (NROW(formula) != NROW(byfactors)) stop("'formula' is the wrong length")
        if (!(is.data.frame(formula) || is.matrix(formula) || is.vector(formula))) stop("invalid type for 'formula'")
      }
      ...
      if (na.rm.all) {
        if (inherits(formula, "formula")) allx <- model.frame(formula, model.frame(design), na.action = na.pass) else allx <- formula
      ...
      if (keep.var) {
        unwrap <- function(x) {
          ...
          rval <- c(rval,
                    c(se = SE(x),
                      ci_l = confint(x)[, 1], ci_u = confint(x)[, 2],
                      cv = cv(x, warn = FALSE),
                      `cv%` = cv(x, warn = FALSE) * 100,
                      var = SE(x)^2)[rep((nvartype - 1) * (nvar), each = nvar) + (1:nvar)])
          if (!is.null(attr(x, "deff"))) rval <- c(rval, DEff = deff(x))
          rval
        }
        results <- (if (multicore) parallel::mclapply else lapply)(uniques, function(i) {
          ...
          if ( covmat || return.replicates ) {
            FUN(data, design[byfactor %in% byfactor[i], ], deff = deff, ..., return.replicates = TRUE)
          } else {
        ...
        if ((covmat && inherits(design, "svyrep.design")) || return.replicates) {
          ...
          covmat.mat <- svrVar( replicates,
                                scale = attr( design , "full_design" )$scale ,
                                rscales = attr( design , "full_design" )$rscales ,
                                mse = attr( design , "full_design" )$mse ,
                                coef = as.vector( sapply( results, coef ) ) )
        } else {
        ...
        unwrap2 <- function(x) {
          if (is.list(x)) x <- x[[1]]
          if (!is.null(attr(x, "deff"))) c(statistic = unclass(x), DEff = deff(x))
          else c(statistic = unclass(x))
        }
        rval <- sapply( uniques , function(i) {
          if (verbose) print(as.character(byfactor[i]))
          if (inherits(formula, "formula")) data <- formula else data <- subset(formula, byfactor %in% byfactor[i])
          unwrap2(FUN(data, design[byfactor %in% byfactor[i], ], deff = deff, ...))
        } )
      ...
      if (nr > 1) rval <- cbind(byfactors[uniques, , drop = FALSE], rval) else rval <- cbind(byfactors[uniques, , drop = FALSE], statistic = rval)
      ...
      if (drop.empty.groups) {
        if (keep.names) rownames(rval) <- paste(byfactor[uniques])
        rval <- rval[order(byfactor[uniques]), ]
        i <- expand.index(order(byfactor[uniques]), nstats)
        if (keep.var) covmat.mat <- covmat.mat[i, i]
      } else {
        ...
        rval <- a
        if (keep.names) rownames(rval) <- levels(byfactor)
        if (keep.var) {
      ...
      attr(rval, "svyby") <- list(margins = 1:NCOL(byfactors),
                                  ...
                                  vartype = vartype)
      if (!keep.names) rownames(rval) <- 1:NROW(rval)
      attr(rval, "call") <- sys.call()
      class(rval) <- c("svyby", "data.frame")
      if (covmat) {
        rownames( covmat.mat ) <- colnames( covmat.mat ) <- names( coef( rval ) )
        attr( rval, "var" ) <- covmat.mat
      }
      if (return.replicates) {
        replicates <- replicates[ , i ]
        colnames( replicates ) <- names( coef( rval ) )
        attr(rval, "replicates") <- replicates
      }
    }

    ### linearization-based objects
    if ( "survey.design" %in% class( design ) ) {
      ...
      if (keep.var) {
        unwrap <- function(x) {
          ...
          rval <- c(rval, c(se = SE(x),
                            ci_l = confint(x)[, 1], ci_u = confint(x)[, 2],
                            cv = cv(x, warn = FALSE), `cv%` = cv(x, warn = FALSE) * 100,
                            var = SE(x)^2)[rep((nvartype - 1) * (nvar), each = nvar) + (1:nvar)])
          if (!is.null(attr(x, "deff"))) rval <- c(rval, DEff = deff(x))
          rval
        }
        results <- (if (multicore)
          parallel::mclapply
            ...
            if (covmat || influence) {
              ...
              idx <- rownames( attr( design , "full_design" )$variables ) %in% rownames( attr( r , "influence" ) )
              attr(r, "index") <- idx
            } else {
              r <- FUN(data, design[byfactor %in% byfactor[i],
              ], deff = deff, ...)
      ...
      if (influence) inflmat <- inflmat[, i]
      attr(rval, "svyby") <- list(margins = 1:NCOL(byfactors),
                                  nstats = nstats, vars = if (keep.var) length(vartype) else 0,
                                  deffs = deff, statistic = deparse(substitute(FUN)),
                                  variables = names(rval)[-(1:NCOL(byfactors))][1:nstats],
      ...
      if ( influence ) {
        rownames( inflmat ) <- rownames( attr( design , "full_design" )$variables )
        colnames( inflmat ) <- names( coef( rval ) )
